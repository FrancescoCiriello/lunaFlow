...
% The _snapshotApp_ function works within the camFlowAqApp to
% acquire a single frame from the
% active cameras as ticked in checkboxes of the active cameras tab
% (app.CheckBox_*.Value = 1). It stores the frame into app.cam1.snapshot
%
% Developed by F. Ciriello 15/7/2019.

fprintf('\nSnapshot AQ launched... \n')
numframes = 1;
...
if app.CheckBox_1.Value == 1 && app.CheckBox_2.Value == 0 && app.CheckBox_3.Value == 0 && app.CheckBox_4.Value == 0
    ...
    [frames, timestamps] = getdata(app.cam1.camera.vid, app.cam1.camera.vid.FramesPerTrigger);

    % save into app temp memory
    app.cam1.snapshot.frames = frames;
    app.cam1.snapshot.timestamps = timestamps;
    
    % save into temp folder
    imwrite(frames, [app.projectfolderEditField.Value, '\temp\cam1\snapshot.bmp'])
    fprintf(['\ncam1 image stored in temp.\n'])
...
if app.CheckBox_1.Value == 0 && app.CheckBox_2.Value == 1 && app.CheckBox_3.Value == 0 && app.CheckBox_4.Value == 0
    ...
    % get frames out of volatile memory
    [frames, timestamps] = getdata(app.cam2.camera.vid, app.cam2.camera.vid.FramesPerTrigger);
    
    % save into app temp memory
    app.cam2.snapshot.frames = frames;
    app.cam2.snapshot.timestamps = timestamps;
    
    % save into temp folder
    imwrite(frames, [app.projectfolderEditField.Value, '\temp\cam2\snapshot.bmp'])
    fprintf(['\ncam2 image stored in temp.\n'])
...
if app.CheckBox_1.Value == 0 && app.CheckBox_2.Value == 0 && app.CheckBox_3.Value == 1 && app.CheckBox_4.Value == 0
    ...
    % get frames out of volatile memory
    [frames, timestamps] = getdata(app.cam3.camera.vid, app.cam3.camera.vid.FramesPerTrigger);
    
    
    % save into app temp memory
    app.cam3.snapshot.frames = frames;
    app.cam3.snapshot.timestamps = timestamps;
    
    % save into temp folder
    imwrite(frames, [app.projectfolderEditField.Value, '\temp\cam3\snapshot.bmp'])
    fprintf(['\ncam3 image stored in temp.\n'])
...
if app.CheckBox_1.Value == 0 && app.CheckBox_2.Value == 0 && app.CheckBox_3.Value == 0 && app.CheckBox_4.Value == 1
    ...
    % get frames out of volatile memory
    [frames, timestamps] = getdata(app.cam4.camera.vid, app.cam4.camera.vid.FramesPerTrigger);
      
    % save into app temp memory
    app.cam4.snapshot.frames = frames;
    app.cam4.snapshot.timestamps = timestamps;
    
    % save into temp folder
     imwrite(frames, [app.projectfolderEditField.Value, '\temp\cam4\snapshot.bmp'])
    fprintf(['\ncam4 image stored in temp.\n'])
